...
export DEFCONFIG=akame-poplar_dsds_defconfig
#export KBUILD_DIFFCONFIG=poplar_dsds_diffconfig
export NKD=AkameKernel-poplar_dsds
...
####---------####
	...
	CC=clang \
	KBUILD_DIFFCONFIG=${KBUILD_DIFFCONFIG} \
	PATH=${PATH} \
...
cp -r arch/arm* Makefile ${OUT_DIR}/Module.symvers ${OUT_DIR}/lib/modules/${KERNEL_NAME}/modules.* ${OUT_DIR}/scripts/mod/modpost ${OUT_DIR}/scripts/genksyms/genksyms  include scripts drivers/misc /${UN}/tmp 
...
####---------####
    ...
    KERNEL_NAME=$(sudo make kernelrelease \
        CC=clang \
        PATH=${PATH} \
        CLANG_TRIPLE=${CLANG_TRIPLE} \
        CROSS_COMPILE=${CROSS_COMPILE} \
...
#===[ COPYNG ]===#
rm ${OUT_DIR}/lib/modules/${KERNEL_NAME}/build
rm ${OUT_DIR}/lib/modules/${KERNEL_NAME}/source
cp -r ${OUT_DIR}/lib/modules/* ${ANYKERNEL_DIR}/modules/system/lib/modules/

cd ${OUT_DIR} 
#cp $(find -name *.ko) --parents ../${ANYKERNEL_DIR}/modules/system/lib/modules/${KERNEL_NAME}/kernel
cd firmware
